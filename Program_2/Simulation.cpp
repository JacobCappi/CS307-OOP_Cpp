...
// ----- Factories init (hanger inits the factories)
    airCraftHanger.init(flightFileName);
...
void Simulation::fetchAircraftInfo(){
    airCraftInstance = airCraftHanger.getAircraft(planeType);
    rateCruise = airCraftInstance->getRateCruise();
    cruisingAlt = airCraftInstance->getRateCruise();
}
...
void Simulation::printFlightDataDepart(FlightData &currentFlight){
    ...
    int realAlt = airCraftInstance->getRealAlt(currentFlight.getStartHour(), currentFlight.getStartMin(), currentHr, currentMin, dis);
    ...
    std::cout << "\n\tCurrent altitude: " << cruisingAlt << " feet";
...
void Simulation::printTime(){
    std::cout << "Current clock time: " << currentHr << ":" << std::setw(2) << std::setfill('0') << currentMin << std::endl;
}
